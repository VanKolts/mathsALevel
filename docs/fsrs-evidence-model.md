...
The dots also came out **elliptical** at first. The SVG used a fixed 300-wide viewBox stretched to fit with `preserveAspectRatio="none"`, which scales x and y by different factors — the path did not care, so the line looked correct, but every `<circle>` was squashed. The viewBox is now measured from the element's real pixel box (one unit = one pixel on both axes) and `preserveAspectRatio` is left at its default, so any residual mismatch letterboxes uniformly instead of distorting. A single `requestAnimationFrame` correction lands the viewBox exactly on the settled box, since the modal is still animating on the frame it opens, and a debounced resize listener rebuilds it if the window changes.
