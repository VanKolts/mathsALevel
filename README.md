...
**Icons.** A 24×24 grid throughout, painted `var(--muted)` and switched to the `#m-rainbow` gradient when active. **The set is mid-migration and currently mixes two styles.** Four are still line-art, stroked: ruled lines for Checklist, then a gear, a bar chart and a 2×2 grid for the tool group. Two are **filled shapes that morph on selection** (`.ic-fill`, added 2026-08-17): a disc for Mistakes and a slab for Past Papers.

The filled pair works the same way. The silhouette lives in a `<mask>`; a knockout inside that mask is what animates, so selecting a tab **resolves the icon into its detail** rather than swapping one glyph for another — Mistakes cuts an X out of the disc (each arm growing from the centre outward along its own axis), Past Papers opens a hole so the slab becomes a page. Three constraints hold this together:
...
- **The mask's children keep their `fill="#000"` / `fill="#fff"` presentation attributes.** The rule above sets `fill:none` on the `<svg>`, which *inherits* into the mask — and inheritance is the weakest step in the cascade, so the presentation attributes win. Move those fills into CSS and every mask inverts.

Timing is 240ms `cubic-bezier(.2,0,0,1)` with a 60ms delay, so the shape settles just as the pill lands and the pill reads as pushing the icon into focus. Deliberately **not** the pill's spring: overshoot on a 24px geometry reads as a wobble. `prefers-reduced-motion` needs nothing extra — the blanket rule in `styles.css` already collapses it to an instant swap.

> **The two styles do not yet sit together.** A filled disc is much heavier at 24px than a 1.85px stroke, so Mistakes and Past Papers currently out-weigh the four line icons beside them. This is a known intermediate state while the set is redrawn, not a finished look. The Focus timer used to count down inside its own button in the tab bar; with that gone, the **Tools icon** carries a pulsing dot (`.rail-live` / `.rail-paused`) so a running timer is still visible once its overlay is closed.
