...
- **It is deliberately *not* a `.m-nav-btn`.** That class carries the icon-morph and colour machinery — `.ic-rain` sits at `opacity: 0` until `.active`, and `positionMNavPill()` measures `.active` — none of which applies to a row that never activates. The label-injection loop would also push a stray `.m-nav-lbl` into a row that has its own. It shares `railAct()` instead, which is the part worth sharing.
...
- **It wears the selector pill's own tint** — `linear-gradient(160deg, var(--spectrum))` at `.17`, the same gradient, angle and opacity `positionMNavPill()` gives the active tab — so the account reads as its own object rather than a fourth nav row, in the app's own colour rather than a grey. The pill itself cannot be reused: it is one shared absolutely-positioned element JS moves to the active tab, and this row never activates. So the tint is a `::before`, which also keeps the opacity off the row's contents. `isolation: isolate` keeps that layer local — without it the stack escapes the rail's context and paints behind the nav's glass. **Keep the gradient and the `.17` in step with `.m-nav-pill`**, or the footer and the selection stop matching. Hover lifts the tint to `.28` rather than recolouring anything, as the pill's rows do. No border: the tint separates it from the rail on its own, which the flat `--surface2` fill it replaced could not (1.08:1).
- **The tint is why the label is `--text` rather than `--muted`.** Measured against the tint composited over `--nav-plate` at both `.17` and `.28`, across every stop of the ramp, `--muted` bottoms out at **3.16:1** (`rose-dark`, on `--spec-3`) and **3.83:1** (`rose-light`) — under the AA floor, and the reason this is checked per-stop is that the background is a gradient, so one sample proves nothing. `--text` holds at 8.55 / 11.91 / 12.37:1. Checking the five stops bounds the whole ramp: a linear gradient keeps each channel between its endpoints, and luminance is monotone in each channel. The person glyph on the full-strength disc measures 6.97 / 5.41 / 6.28:1. The row is 183×52, well clear of [SC 2.5.8](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html).
- **Both SVGs carry `width`/`height` and `fill="none"` as presentation attributes**, not only in CSS. An SVG with a `viewBox` and no intrinsic size fills its container, so a moment of stale CSS renders the row as two full-width solid-black glyphs stacked vertically — which is exactly what shipped for a few minutes on 2026-08-25. See the warning below; the CSS still sets the real values, these are the floor.

> **`CACHE_VERSION` does not control the browser's HTTP cache or the CDN's.** GitHub Pages serves every file `cache-control: max-age=600`, and they expire independently of one another. The service worker is network-first for own-origin files, but that `fetch()` still goes through the HTTP cache — so a fresh `index.html` can arrive next to a ten-minute-old `styles.css`, and a bump changes nothing about it. Any change that pairs new markup with new styling has that window. Degrading gracefully is the mitigation, not the cure — the cure is [versioned asset URLs](#known-gaps--roadmap), which is on the roadmap and not yet done.
...
They render as nothing, so the UI looks finished; `?icons` on the URL outlines every one in place. A handful marked `ico-live` sit where the emoji *was* the control — the favourite heart, the note marker — and draw a neutral box instead, because rendering nothing would delete the affordance; those carry `data-state="on|off"`. `ico(name, extra, state)` generates the markup for the JS-built ones, and `lnk()` on paper cards now takes an icon **name** rather than a glyph. **[`docs/icon-placeholders.md`](docs/icon-placeholders.md) lists all 39 sites across 25 distinct icons** — so the icon job is **25 drawings**, not the 46–48 the docs claimed for months. It is generated by [`scripts/gen-icon-index.mjs`](scripts/gen-icon-index.mjs); `npm run icons` rewrites it and `npm test` fails if it lists an icon the source no longer has. That check exists because the file said "this file is generated" for months with no generator behind it, and had drifted to list `clipboard` and `document`, both deleted. Its **line numbers still go stale fast**, so treat the icon *names* as the durable handle.
...
**Icons.** A 24×24 grid throughout, rendered at 28px, painted `var(--muted)` and switched to the `#m-rainbow` gradient when active. **Every icon in the rail is now a filled shape** (`.ic-fill`); the stroked line-art set is gone. The three page tabs morph on selection: a rhombus for Checklist, a disc for Mistakes, a slab for Past Papers. The two tool icons are **one state only** (`.ic-solid`) — a rounded triangle for Statistics, a rounded square for Tools — because they open overlays and never become the active page, so there is nothing for a knockout to morph into and no rainbow to arrive.

> **One state means the shape itself can be painted.** The five-layer mask construction below exists to stop two antialiased edges multiplying and to keep the pill's gradient from showing through a knockout — neither applies when nothing animates and there is no hole, so `.ic-solid` is a single `<path>` or `<rect>` with a `fill`. The triangle's corners are arcs on the path rather than a stroke trick, so the silhouette stays the size it is drawn at: the tangent length for a 60° vertex at `r = 2` is `r / tan(30°) = 3.464`, which is where its coordinates come from. The `.rail-live` timer state had to move from `stroke` to `fill` with them — the old rule coloured a stroke the square no longer has.
...
**Only the pill travels; the colour fades in on the spot.** `.ic-rain` sits behind the silhouette at `opacity: 0` and goes to `1` when the button becomes active — `.ic-dim`, the tinted interior layer, settles at `.35` on the same curve. Nothing translates. The icon being left fades out on its own the moment `.active` is removed, with no class and no timer to clean up. Because a fade has no axis, **one rule covers both orientations** — the mobile query used to restate the whole thing in `translateX`, and no longer restates anything.

> **This replaced a colour *flow* on 2026-08-25, and the simplification is the point.** The rainbow used to slide in from the side the pill was arriving from, so the colour read as being carried between icons. Making that work took a `--nav-dir` custom property, a forced style flush to resolve the incoming icon's parked position against the new direction, a `.nav-leaving` class on the outgoing button, a release timer, and a standing rule that an idle icon must never carry a transition (its parked position depended on the variable, so flipping it would animate the rainbow straight across every idle face at once). It also produced two real bugs — the colour entering from the wrong side on every switch after the first, because `--nav-dir` and `.active` were set in the same task and the transition's "before" snapshot was still the previous frame's; and `.nav-leaving` and `.active` landing on the same button when you returned to a tab inside the release window, where equal specificity meant source order won and the icon sat parked off-screen until the timer fired. **All of that machinery existed to serve a direction. A fade has no direction, so all of it went** — about twenty lines of JS and three CSS rules, including both bug fixes, because neither failure can be expressed any more.

Timing: the pill 400ms `cubic-bezier(.32,1.08,.5,1)` with **no delay**, then shape and colour both 400ms `cubic-bezier(.2,0,0,1)` with a **180ms delay**. So the selector arrives first and the icon resolves just behind it — the colour used to start at 0ms alongside the pill, which is what made two things appear to move at once. Measured at ×10 through test mode's motion control: at 1790ms the pill had covered 51 of its 56px and the incoming rainbow was still at 0; it began at ~1800 and reached 0.9 by 4000. The durations are unchanged. The pill's overshoot was cut from 1.4 to 1.08 because the old curve visibly bounced past the row and settled back. Deliberately **no** overshoot on the icon geometry, where it reads as a wobble rather than a spring. `prefers-reduced-motion` needs nothing extra — the blanket rule in `styles.css` already collapses all of it to an instant swap. The Focus timer used to count down inside its own button in the tab bar; with that gone, the **Tools icon** carries a pulsing dot (`.rail-live` / `.rail-paused`) so a running timer is still visible once its overlay is closed.
...
| One gutter, not two | `--wrap` is the *only* horizontal inset | `header` and `.filters-bar` each added 24px of their own padding on top of it, so the title and the filter chips sat 24px inside the content column they head — the app's columns did not line up. Those bars keep vertical padding and the safe-area insets only |
...
**The two segmented controls span their column too.** Fixing the containers was necessary but not sufficient: `.mv-tabs` (Log / Topics / Reasons / Trend / Leaks) was `width: fit-content` and `.pp-type-seg` (the paper modules) was `display: inline-flex`, so both shrink-wrapped their labels. On a 1440px window that left them at **310px and 611px against 1136px** of content beside them — the most visible way the two pages failed to match the Checklist, since every other section on those pages already spanned. They are `width: 100%` / `display: flex` now, with their buttons growing to share the room and never shrinking below their own text, so a column too narrow for all six scrolls instead of crushing them.

> **A full-width strip has to keep its pill.** All three of these controls (the two above plus `#trend-range-tabs`) position their pill in JS from the active button's `offsetLeft`/`offsetWidth`, and **nothing repositioned them on a resize.** That never showed while they were content-sized — their width barely moved — but a strip that spans its column tracks the window, so every resize stranded the pill. A single `ResizeObserver` over the three strips fixes it, chosen over a resize listener for the same reason the exam caches key off a signature rather than an invalidate call: it catches every cause — a window resize, the orientation flip between rail and bottom bar, a webfont finishing and re-measuring the labels — not just the one somebody remembered. The pill is absolutely positioned inside the strip, so moving it cannot resize the strip and feed the observer its own output.

**All three pages share one container.** Checklist's `<main>` and the `.tab-content` wrapper used by Mistakes and Past Papers had drifted apart: `main` took `--wrap` (up to 1360px) while `.tab-content` was capped at `max-width: 1060px` with 24px of its own horizontal padding. On a 1440px window that made Mistakes and Past Papers **300px narrower than Checklist**, and inset differently. They are one selector now (`main, .tab-content`) rather than two declarations that happen to match, at all three breakpoints the container is styled at — base, ≤640px and print. Measured after: header, filter bar, review panel and all three pages come out at exactly `w=1136, L=256, R=48` on a 1440px window, and `w=582, L=16, R=16` on a phone.
...
> **The worked solution is readable in the log** (2026-09-19). A detailed log captures `m.answer`, but the list rendered only a *"🔁 re-attemptable"* badge — the solution itself was reachable from the re-attempt card at the top of the page and nowhere else, so browsing your own library showed the mistake and not the fix. Each card now carries a **`▸ Worked solution`** disclosure.
>
> **Collapsed by default**, which is the only real design question here: the log is also where you go to *find* something to redo, and a solution printed inline spoils every question you scroll past. Open cards are remembered in `mkOpenSolutions` (module scope, not a class on the node) so changing the sort or grouping — which rebuilds every card — does not close what you just opened; the set is pruned when an entry is deleted, when its logbook is deleted, and when an edit removes the answer. The body is `white-space: pre-wrap`, because working is written in lines and a solution reflowed into one paragraph is unreadable however well it was typed.
>
> The **question and the worked solution are now editable** too. Both fields are in the edit dialog, always shown rather than only for entries that already have them, so a quick log can be written up into a full record later — which is when a student usually knows the answer. Blank clears the field rather than storing `''`, since `m.answer` is what makes an entry re-attemptable.
>
> The *"re-attemptable"* badge also **stopped over-claiming**: it appeared for any entry with an answer, while `reattemptPool()` requires `answer && (qImgId || img)`, so written solutions without a photo advertised a loop they could never appear in. It matches the pool now.

> **A silent topic-blanking bug, found while testing the above.** `#em-topic-sel` lists only the topics of the track you are on. Assigning a `<select>` a value it does not have sets its value to `''` — so opening any mistake whose topic was off-track (logged on Further Maths, viewed on Maths; or renamed since) and pressing **Save** wrote that empty string back, detaching the record from its topic. On a Further-Maths-seeded store that was **135 of 400** entries, one Save away each. Two fixes, deliberately both: the dialog now inserts the record's own topic as an option labelled *"— not in your current track"*, and the save path treats an empty select on a record that already had a topic as *"the dialog could not represent it"* rather than *"the student chose none"* — there is no way to choose none, so it keeps the existing value. Only a record genuinely arriving without a topic is asked for one. Verified by sabotage: deleting the preserved option before saving still round-trips the topic.
...
**The Topic Breakdown collapses.** With a full syllabus on the go it is 200+ rows in one unbroken scroll, which buried the per-cluster progress bars — the part you actually scan. Each cluster's header is now a toggle (`_statsOpenClusters`, delegated once because `renderStats()` replaces the whole body on every draw); the bar and the counts stay visible either way, so nothing you were reading is hidden. `display:none` rather than a height animation: animating 200 rows is a jank machine, and the panel has enough motion already.
...
- **📅 Revision plan / My exams** — the multi-exam adaptive planner. Instead of one global exam date, you set a date **per paper** (AS P1–P2, A-Level P1–P3, and the Further modules), seeded from `exam-dates.json` (currently the provisional Edexcel Summer 2027 timetable) and overridable per paper. Each topic then ramps against the date of the paper that actually examines it, so Statistics tightens for the Stats paper rather than for whichever exam happens to be first. Reachable from Settings → My exams, from the Tools overlay, and from a plan generator on the Checklist quick-row.
  ...
  **How the two halves fit together**, since the window is easy to misread as doing more than it does:
  ...
  The ramp itself is three lines of `targetRetention()`: outside `EXAM_RAMP_DAYS` (70) the target is `BASE_RETENTION` (0.90); inside it, it interpolates linearly to `MAX_RETENTION` (0.97) at the exam. Higher target ⇒ shorter intervals ⇒ topics resurface faster. Once **every** paper assessing a component has been sat, `examDateForComponent()` returns `''` and that component drops back to `BASE_RETENTION` — revision after the exam is ordinary revision. Alongside it, `needsExamConfirmation()` forces one confirmation pass over anything not reviewed since the run-in began, so a topic stable enough to sail past the exam date still gets seen once; **that one still arrives as a cliff** — see [Known gaps](#known-gaps--roadmap).

  Walking a Pure topic through the A-Level timetable, which is the clearest way to see all of it at once:

  | Date | Ramps toward | Target retention |
  |---|---|---|
  | 8 months out | Paper 1 · Pure 1 | 90.0% (idle) |
  | 7 days out | Paper 1 · Pure 1 | 96.0% |
  | 26 May, Paper 1 | Paper 1 · Pure 1 | 96.7% (ceiling) |
  | 27 May | **Paper 2 · Pure 2** | 95.4% — off the ceiling, re-ramping |
  | 9 June, Paper 2 | Paper 2 · Pure 2 | 96.7% |
  | 16 June onward | **nothing left** | 90.0% — released |
...
Changing tab dropped frames — on the heavy seed (315 topics, 400 mistakes, 60 papers; 10,657 nodes), two dropped frames totalling **183ms** out of a 400ms animation. It read as the animation freezing and then catching up. Three causes, none of them the animation's own timing.

> **The numbers below are all deterministic — main-thread time and forced-layout time, each the median of three runs.** Frame-interval sampling was tried first and abandoned: it runs on the main thread, so it cannot observe the compositor animation it is meant to be judging, and the preview harness throttles `requestAnimationFrame` between tool calls, which produced fictitious 1,800ms "frames". A measurement that cannot distinguish a real stall from an idle tab is not a measurement.

| | Before | After |
|---|--:|--:|
| Work in the task that starts the transition (`checklist → mistakes`) | 47–70ms | **8ms** (23ms cold) |
| Same, `mistakes → papers` / `papers → checklist` | 13ms / 8ms | **2.5ms / 4.3ms** |
| Forced relayout of the Mistakes page | ~50ms | **~10ms** |
...
- **The destination's render ran in the same task as the style change.** A CSS transition does not begin when you change the style — it begins at the first frame **painted** afterwards, when the browser assigns its `startTime` and hands it to the compositor. So 47–70ms of `renderMistakesTab()` in that same task simply delayed the start; the animation was not janky so much as late. Verified directly: blocking the thread for 250ms straight after the style change leaves the transition at `startTime: null`, `currentTime: 0` — not begun at all.
  ...
  > **`requestAnimationFrame` alone does not fix this, and that is the trap.** rAF callbacks run *before* the paint, so deferring the render by one rAF still lands it inside the window where the transition has not started. It must be `requestAnimationFrame(() => setTimeout(fn, 0))` — a timeout scheduled from inside a rAF runs after that frame is painted.

- **6,750 nodes laid out in the frame the tab became visible.** The mistakes list is not paginated: a full log renders every card. `.m-card` carries `content-visibility: auto`, which takes the page's forced relayout from ~50ms to ~10ms.

  > **`contain-intrinsic-size` is a content-box height, and getting that wrong is silent.** The cards measure a mean of 79.4px on screen, so 73px looked like a reasonable estimate — but that figure is the *border* box, and `.m-card` carries 24px of padding and 2px of border. Every un-rendered card was over-estimated by ~26px, making the document 42,612px against a true 35,064: **a scrollbar 21% longer than the page**, with nothing on screen looking wrong. At `auto 53px` the error is **−0.55%**, and a full scroll produces no drift at all. Deliberately a shade under rather than over, so the page grows as you scroll rather than shrinking — shrinking below the fold is what moves the view under you.

Two correctness fixes came out of the same work. `switchTab` carries a **sequence guard** so that tapping through three tabs quickly cannot let a superseded render paint into a page nobody is on. And the `prefers-reduced-motion` block now zeroes `transition-delay` as well as duration — collapsing the duration alone left the icon's 180ms delay intact, which is not less motion, it is the same motion with a stutter in front of it.

> **The sequence guard was itself broken for its first commit, and it broke the thing it guarded.** It was written as `switchTab._seq`, but the app-shell IIFE does `var _origSwitch = switchTab; switchTab = function(…)`, so by the time the body runs the global `switchTab` is the *wrapper* — a different object from the one the initialiser attached to. `++switchTab._seq` was therefore `++undefined` = `NaN`, and the guard compared `NaN !== NaN`, which is always true. **The deferred render never ran once:** Past Papers rendered as an empty page, and the first round of "zero dropped frames" measurements were taken across a switch that did no work at all. The counter is a plain module-scope `let` now. Two lessons, both cheap: a property on a function that something else reassigns is not state, and a performance result that improves suspiciously far is a reason to check the feature still works.
...
| **Motion** | `1× / ¼× / ⅒× / 0×`. The nav transition is 400ms behind a 180ms delay, and the ordering — pill first, icon behind it — is not something you can judge at full speed. ⅒× is how the sequencing was measured. `0×` also exercises the branch the blanket `prefers-reduced-motion` rule takes, otherwise untestable without changing an OS setting |
| **See the layout** | Magnify the nav ×3 for pixel work on the icons; outline every box (rows and their children in separate hues); outline the 39 named icon placeholders, the same thing `?icons` does |
...
- **Local assets are unversioned, so a deploy has a ten-minute skew window.** Pages serves every file `cache-control: max-age=600` and they expire independently, so a visitor can get a fresh `index.html` beside a stale `styles.css` or a stale `data/*.js` — measured on 2026-08-25 at `age: 42` for the stylesheet against `age: 0` for the markup. `CACHE_VERSION` has no bearing on it: it names the *service worker's* cache, and the worker's network-first `fetch()` for own-origin files still passes through the browser's HTTP cache. Today the only defence is that each component degrades gracefully on its own.

  The fix is a version query on every local asset, kept in step by the deploy script. Six references, all of which must move together or the precache stops matching what the page requests:

  | File | Change |
  |---|---|
  | [`index.html`](index.html) | `<link rel="stylesheet" href="styles.css?v=NN">` and the five `<script src="data/*.js?v=NN">` tags |
  | [`sw.js`](sw.js) | the matching `SHELL` entries — a precache of `./styles.css` never satisfies a request for `./styles.css?v=39` |
  | [`scripts/deploy.sh`](scripts/deploy.sh) | rewrite all of them in the same step that bumps `CACHE_VERSION`, so the two can never disagree |

  Two things to get right: `sw.js` itself must stay unversioned (nothing can rewrite the URL the browser fetches it from), and `scripts/validate.mjs` checks that every file `index.html` references and `sw.js` precaches actually exists — it will need to strip the query before resolving the path, or every asset reads as missing.
