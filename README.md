...
- **It is a card, not a bare row** — `--surface2` fill, `--border2` hairline, `--r-md` corners — so the account reads as its own object rather than a fourth nav row. The hairline is load-bearing rather than decoration: the fill measures only **1.08 / 1.11 / 1.23:1** against the rail behind it across `rose-dark` / `rose-light` / `pure-white`, so on its own it barely separates. Text and icon sit at 5.66 / 5.57 / 7.58:1 resting and 15.34 / 17.34 / 17.14:1 on hover, all clear of AA. The row is 183×52, well clear of [SC 2.5.8](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html).
...
> **`CACHE_VERSION` does not control the browser's HTTP cache or the CDN's.** GitHub Pages serves every file `cache-control: max-age=600`, and they expire independently of one another. The service worker is network-first for own-origin files, but that `fetch()` still goes through the HTTP cache — so a fresh `index.html` can arrive next to a ten-minute-old `styles.css`, and a bump changes nothing about it. Any change that pairs new markup with new styling has that window. Mitigated here by making the markup degrade gracefully; the durable fix is a versioned asset URL (`styles.css?v=NN`) rewritten by `deploy.sh` alongside the `CACHE_VERSION` bump, which would also need the `sw.js` precache entry to match.
